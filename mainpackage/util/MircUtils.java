...
public class MircUtils {
	...
	public static javafx.scene.paint.Color getMircColorAsJavaFXColor(int color) {
		Color c = mircColors.get(color);
		return javafx.scene.paint.Color.color(c.getRed() / 255f, c.getGreen() / 255f, c.getBlue() / 255f, c.getAlpha() / 255f);
	}
	...
	public static Integer getMircColorCodeFromColor(javafx.scene.paint.Color color) {
		return getMircColorCodeFromColor(color, -1);
	}
	
	public static Integer getMircColorCodeFromColor(javafx.scene.paint.Color color, int accuracy) {
		return getMircColorCodeFromColor(color, MircColorsStyle._99_COLORS, accuracy);
	}
	
	public static Integer getMircColorCodeFromColor(javafx.scene.paint.Color color, MircColorsStyle style) {
		return getMircColorCodeFromColor(color, style, -1);
	}
	...
	public static Integer getMircColorCodeFromColor(javafx.scene.paint.Color color, MircColorsStyle style, int accuracy) {
		...
		for (int i = 0; i <= accuracy; i++) {
			for (int n = minColors; n < maxColors ; n++) {
				Color awt = mircColors.get(n);
				double r2 = awt.getRed() / 255.0;
				double g2 = awt.getGreen() / 255.0;
				double b2 = awt.getBlue() / 255.0;
				if (accuracy == 0) {
					double dist = Math.pow(r2 - r, 2) + Math.pow(g2 - g, 2) + Math.pow(b2 - b, 2);
					if (dist < dis) {
						dis = dist;
						value = n;
					}
