...
public abstract class CollectionUtils {
	...
	public static <T> T removeLast(List<T> list) {
		return removeLast(list, 0);
	}
	
	public static <T> T removeLast(List<T> list, int decreasedIndexAmountFromLast) {
		if (list == null || list.isEmpty() || list.size() - 1 - decreasedIndexAmountFromLast < 1)
			return null;
		return list.remove(list.size() - 1 - decreasedIndexAmountFromLast);
	}
	...
	/**
	 * Move um item para cima ou para baixo em uma [@code List} (Se {@code inc} for
	 * um valor negativo, move para cima, caso contrário, move para baixo. Ex: Se
	 * informar o valor {@code 3} para {@code inc}, o item será movido 3 posições
	 * para baixo. Se informar o valor {@code -4} para {@code inc}, o item será
	 * movido 4 posições para cima.
	 * 
	 ...
	 * @return Um valor indicando o total de indices que o item foi movida. Ex: Se o
	 *         item estiver no indice 2, e você mandar mover o item 10 indices para
	 *         baixo, irá retornar -2 pois ao mover 2 indices para cima, o item
	 *         chega ao indice 0.
	 */
